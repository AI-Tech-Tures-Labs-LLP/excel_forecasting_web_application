// Merged ProductTable.jsx - with search functionality and notes sorting
...
const ProductTable = ({
  ...
  const filteredProducts = useMemo(() => {
    if (!searchQuery || searchQuery.trim() === "") {
      return currentProducts; // Show all products if no search query
    }
    ...
    return currentProducts.filter((product) => {
      ...
      const productIdMatch = product.product_id
        ?.toString()
        .toLowerCase()
        .includes(query);
      ...
      // Search in product name if available
      const nameMatch = product.name?.toLowerCase().includes(query);

      // Search in SKU if available
      ...
      // Search in assigned user name if available
      let assignedUserMatch = false;
      if (product.assigned_to) {
        if (typeof product.assigned_to === "object") {
          const userFullName = `${product.assigned_to.first_name || ""} ${
            product.assigned_to.last_name || ""
          }`.toLowerCase();
          const username = product.assigned_to.username?.toLowerCase() || "";
          const email = product.assigned_to.email?.toLowerCase() || "";
          assignedUserMatch =
            userFullName.includes(query) ||
            username.includes(query) ||
            email.includes(query);
        } else if (typeof product.assigned_to === "number") {
          const assignedUser = assignedUsers.find(
            (user) => user.id === product.assigned_to
          );
          if (assignedUser) {
            const userFullName = `${assignedUser.first_name || ""} ${
              assignedUser.last_name || ""
            }`.toLowerCase();
            const username = assignedUser.username?.toLowerCase() || "";
            const email = assignedUser.email?.toLowerCase() || "";
            assignedUserMatch =
              userFullName.includes(query) ||
              username.includes(query) ||
              email.includes(query);
          }
        } else if (typeof product.assigned_to === "string") {
          assignedUserMatch = product.assigned_to.toLowerCase().includes(query);
        }
      }

      // Also check in product notes data for assigned users
      const notesData = productNotesData[product.product_id];
      const notesAssignedMatch =
        notesData?.assignedTo?.toLowerCase().includes(query) ||
        notesData?.productAssignedTo?.toLowerCase().includes(query);
  ...
  }, [currentProducts, searchQuery, assignedUsers, productNotesData]);

  // 🔥 Update processedProducts to use filtered results
  ...
  // 🔥 Update selection logic to work with filtered products
  const isAllSelected =
    displayProducts.length > 0 &&
    displayProducts.every((p) => selectedProductIds.includes(p.product_id));
  ...
  // 🔥 Update handleSelectAll to work with filtered products
  const handleSelectAll = (checked) => {
    if (checked) {
      // Select all from filtered/displayed products
      const allProductIds = displayProducts.map((p) => p.product_id);
  ...
  const hasUnassignedProducts = () => {
    return displayProducts.some((product) => !product.assigned_to);
  };
  ...
  // 🔥 Updated to show filtered results count and search context
    ...
    return (
      <div className="text-center py-12 text-gray-500">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          {hasActiveSearch ? "No Search Results Found" : "No Products Found"}
        </h3>
        <p className="mb-4">
          {hasActiveSearch
        ...
        {(hasActiveSearch || hasActiveFilters) && (
          <button
            onClick={() => {
              // Clear search and filters logic would go here
              // This would typically be handled by the parent component
            }}
            className="text-indigo-600 hover:text-indigo-800 font-medium"
          >
            Clear {hasActiveSearch ? "search" : "filters"}
          </button>
        )}
      </div>
    );
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-blue-800">
                Showing {displayProducts.length} results for "
                <strong>{searchQuery}</strong>"
              </span>
            </div>
            <button
              onClick={() => {
                // This would typically call the parent's search clear function
      ...
      <div className="overflow-x-auto min-h-[500px]">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {/* 🔥 Use displayProducts (filtered) instead of currentProducts */}
