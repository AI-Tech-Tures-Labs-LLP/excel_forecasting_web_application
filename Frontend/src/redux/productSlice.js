...
export const fetchProducts = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      multiSelectFilters.forEach((filterKey) => {
        ...
        if (Array.isArray(values) && values.length > 0) {
          values.forEach((value) => {
            ...
            } else if (filterKey === "birthstone") {
              // FIXED: Ensure birthstone values are always uppercase
              const uppercaseValue = value.toString().trim().toUpperCase(); // ✅ Ensure full uppercase
              params.append(filterKey, uppercaseValue);
              console.log(
                `✅ Added birthstone parameter: ${filterKey}=${uppercaseValue}`
              );
            } else {
      ...
      if (filters.final_qty_sort && !filters.sort_by) {
        params.append(
          "sort_by",
      ...
      if (filters.last_reviewed_sort && !filters.sort_by) {
        params.append(
          "sort_by",
...
const productSlice = createSlice({
  ...
  reducers: {
    ...
    setSorting: (state, action) => {
      ...
      const mappedSortBy = sortKeyMapping[sortBy] || sortBy;

      state.appliedFilters.sort_by = mappedSortBy;
    ...
    builder
      ...
      .addCase(fetchProducts.fulfilled, (state, action) => {
        ...
        console.log("Final product counts:", {
          ...
          sorting: {
            sort_by: state.appliedFilters.sort_by,
            sort_direction: state.appliedFilters.sort_direction,
          },
        });
      })
...
export const selectIsFieldSorted = (state, fieldName) => {
  ...
  return currentSort.sortBy === mappedField;
};
...
export const selectProductsDebug = (state) => ({
  ...
  sortingDetails: {
    sort_by: state.products.appliedFilters.sort_by,
