...
from forecast.service.staticVariable import OMNI_RENAME_MAP, COM_RENAME_MAP, STORE_RENAME_MAP,MONTH_DATA, MONTH_COLUMN_MAP
import calendar
...
def process_data(input_path, file_path, month_from, month_to, percentage, input_tuple,  sheet_object, current_date):
   ...
   # For OmniForecast - Updated with all the fields matching the model definition
    ...
    file_path = os.path.join(settings.MEDIA_ROOT, "forecast_summary.xlsx")
...
def process_category(args):    
    ...
    for loop in range(num_products):
        ...
        macys_receipt_matching_row=df_outputs['macys_receipts'].loc[df_outputs['macys_receipts']['PID'].str.upper() == pid_value]
        # Find the matching rows
        logging.info(f"pid_value: {pid_value}")
        loader = VariableLoader(cross_ref,matching_row,macys_receipt_matching_row,df_outputs['index_df'],df_outputs['all_data'],df_outputs['mcom_data'],std_period,year_of_previous_month,last_year_of_previous_month)
        ...
        def extract_product_type(pid_type):
            print(f"Extracting product type from pid_type: {pid_type}")
        ...
        if pid_type == 'com_pid' and not pid_omni_status:
            ...
            value = loader.TY_MCOM_OH_Units[current_month]+planned_shp[current_month]-planned_fc[current_month]
