...
class DataFrameBuilder:
    def __init__(self, sheets, return_QA_df):
        self.sheets = sheets
        self.return_QA_df = return_QA_df
        self.index_df = None
        self.report_grouping_df = None
        self.planning_df = None
        self.master_sheet = None
        self.vendor_sheet = None
        self.TBL_Planning_VerticalReport__3 = None
        self.Macys_Recpts = None
        self.All_DATA = None
        self.MCOM_Data = None

    def build(self):
        self._prepare_return_QA_df()
    ...
    def get_outputs(self):
        return {
            ...
            "TBL_Planning_VerticalReport__3": self.TBL_Planning_VerticalReport__3,
            "Macys_Recpts": self.Macys_Recpts,
            "All_DATA": self.All_DATA,
            "MCOM_Data": self.MCOM_Data,
            "birthstone_sheet": self.birthstone_sheet,
            "return_QA_df": self.return_QA_df
        }

    def _prepare_return_QA_df(self):
        self.return_QA_df = self.return_QA_df.rename(columns={
            'Item #': 'PID',
            'Expected Return Period': 'Expected Return',
            'Total Qty to Return': 'Quantity'
        })
        ...
        self.return_QA_df['Month'] = self.return_QA_df['Expected Return'].apply(extract_month)
        
        self.return_QA_df = self.return_QA_df.groupby(['PID', 'Month'], as_index=False)['Quantity'].sum()

    def _prepare_index_df(self):
        index_df_raw = self.sheets["Index"]
        index_df = index_df_raw.iloc[1:43, :16]
        index_df.columns = index_df_raw.iloc[1]
        index_df = index_df.reset_index(drop=True).drop(index_df.index[0]).reset_index(drop=True)
        self.index_df = index_df

    def _prepare_report_grouping(self):
        self.report_grouping_df = pd.DataFrame(
            self.sheets["report grouping"].values[2:],
            columns=self.sheets["report grouping"].iloc[1].values
        )

    def _prepare_planning_df(self):
        self.planning_df = pd.DataFrame(
            self.sheets["Repln Items"].values[2:],
            columns=self.sheets["Repln Items"].iloc[1].values
        )

    def _prepare_additional_sheets(self):
        self.TBL_Planning_VerticalReport__3 = pd.DataFrame(
            self.sheets["Setup Sales -L3M & Future"].values[9:],
            columns=self.sheets["Setup Sales -L3M & Future"].iloc[8].values
        )
        self.Macys_Recpts = pd.DataFrame(
    ...
    def _build_master_sheet(self):
        df_filtered = self.planning_df.copy()

        df_filtered['Gender'] = 'Women'
        df_filtered.loc[df_filtered['Dpt ID'].isin([768, 771]), 'Gender'] = 'Men'
        df_filtered.loc[df_filtered['CL ID'] == 86, 'Gender'] = 'Children'
        ...
        def categorize_product(class_desc):
            if isinstance(class_desc, str):
                ...
                if 'EARRINGS' in class_desc or 'EARS' in class_desc:
                    ...
                    return 'Pendants'
                elif ' RING' in class_desc:
                    return 'Rings'
                elif 'BRACELETS' in class_desc:
                    return 'Bracelets'
                elif 'CHAIN' in class_desc or 'NECKS' in class_desc or 'NECKLACE' in class_desc:
                    return 'Necklace'
                elif 'SET' in class_desc:
                    return 'Set'
                elif 'BAND' in class_desc:
        ...
        self.master_sheet = df_filtered[[
            'PID', 'category', 'Birthstone', 'BSP_or_not', 'type', 'Gender',
            'Vendor', 'Vendor Name', 'Own Retail', 'FC Index', 'FLDC', 'Safe/Non-Safe', 'Item Code'
        ]]
    ...
    def _build_vendor_sheet(self):
        df_unique_vendor = self.df_filtered[['Vendor', 'Vendor Name']].drop_duplicates().dropna()
        ...
        df_coo = pd.DataFrame(data)
        self.vendor_sheet = pd.merge(df_unique_vendor, df_coo, on='Vendor Name', how='left')
